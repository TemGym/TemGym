...
def lens_phase_factor(n, ps, lambda0, f):
    ...
    phase_factor = jnp.exp(-1j * jnp.pi * (X**2 + Y**2) / (lambda0 * f) + 1j * jnp.pi)

    return phase_factor

@jax.jit
def run_model_for_rays_and_slopes(transfer_matrices, input_slopes, scan_position):
    # Given input model and its transfer matrix, run the model to find the positions of the rays at the sample and detector
    ...
    rays_at_source_with_semi_conv = jnp.vstack([
        jnp.full((input_slopes_x.shape[0],), scan_pos_x),
        jnp.full((input_slopes_y.shape[0],), scan_pos_y),
        input_slopes_x,
        input_slopes_y,
...
def find_input_slopes_that_hit_detpx_from_pt_source_with_semiconv(
    ...
    mask = F <= 0
    input_slopes_masked = jnp.stack([theta_x_in, theta_y_in]) * mask
